		...
		case SEMDB_DATA_FILES:
			{
				...
				fname := items[1]
				if key == "formDictFile" {
					formFile = path + "/" + strings.Replace(fname, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/", "", -1)
				} else if key == "senseDictFile" {
					dictFile = path + "/" + strings.Replace(fname, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/", "", -1)
				} else if key == "wnFile" {
					wnFile = path + "/" + strings.Replace(fname, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/", "", -1)
				}
				break
			}
		default:
...
func (this *SemanticDB) getWordSenses(form string, lemma string, pos string) *list.List {
	...
	for p := searchList.Front(); p != nil; p = p.Next() {
		LOG.Trace("github.com/sniperkit/xanalyze/plugin/nlp/freeling.searching " + p.Value.(Pair).first.(string) + " " + p.Value.(Pair).second.(string))
		...
		if lsen.Len() > 0 {
			LOG.Trace("github.com/sniperkit/xanalyze/plugin/nlp/freeling.senses found " + strings.Join(StrList2StrArray(lsen), " "))
		}
	}
