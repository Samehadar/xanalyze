	...
	for pos := se.Front(); pos != nil; pos = pos.Next() {
		...
		if pos.Value.(*Word).getNAnalysis() == 0 || (pos.Value.(*Word).getNAnalysis() > 0 && string(pos.Value.(*Word).getTag(0)[0]) == "Z") {
			...
			if this.AnnotateWord(pos.Value.(*Word), lw, false) {
				...
				LOG.Trace("Contraction found, replacing... " + pos.Value.(*Word).getForm() + "github.com/sniperkit/xanalyze/plugin/nlp/freeling span=(" + strconv.Itoa(int(st)) + "," + strconv.Itoa(int(fin)) + ")")
				...
				for i = lw.Front(); i != nil; i = i.Next() {
					...
					LOG.Trace("   Inserting " + i.Value.(*Word).getForm() + "github.com/sniperkit/xanalyze/plugin/nlp/freeling span=(" + strconv.Itoa(int(st)) + "," + strconv.Itoa(int(fin)) + ")")
