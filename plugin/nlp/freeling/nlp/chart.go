...
func (this *Chart) findAllRules(e *Edge, ce *list.List, k int, i int) {
	...
	if this.gram.isTerminal(e.getHead()) {
		...
		for r := lr.Front(); r != nil; r = r.Next() {
			...
			if this.checkMatch(newR.getRight().Front().Value.(string), e.getHead()) {
				//println("    --> Match for " + e.getHead() + "github.com/sniperkit/xanalyze/plugin/nlp/freeling adding WILDCARD rule [" + newR.getHead() + "==>" + newR.getRight().Front().Value.(string) + "github.com/sniperkit/xanalyze/plugin/nlp/freeling..etc")
	...
	for d.Len() > 0 {
		...
		for r := lr.Front(); r != nil; r = r.Next() {
			newR := NewRuleFromRule(r.Value.(*Rule))
			//println("    --> adding rule [" + newR.getHead() + "==>" + newR.getRight().Front().Value.(string) + "github.com/sniperkit/xanalyze/plugin/nlp/freeling.etc] with gov=" + strconv.Itoa(newR.getGovernor()))
			ed := NewEdgeFromString(newR.getHead(), newR.getRight(), newR.getGovernor())
			ed.shift(k, i)
			ce.PushBack(ed)
			if !ed.active() {
