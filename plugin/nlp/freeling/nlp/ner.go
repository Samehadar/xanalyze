...
func (this *NP) ComputeToken(state int, j *list.Element, se *Sentence) int {
	...
	if ignore == 2 {
		LOG.Trace("Ignorable word (" + form + If(it != 0, ","+itKey+")", ")").(string) + "github.com/sniperkit/xanalyze/plugin/nlp/freeling Ignore = 0")
		if state == NP_ST_NP {
			token = NP_TK_mUpper
		} else {
			nxt := j
		...
		LOG.Trace("Ignorable word (" + form + If(it != 0, ","+itKey+")", ")").(string) + "github.com/sniperkit/xanalyze/plugin/nlp/freeling Ignore = 1")
	} else if sbegin {
