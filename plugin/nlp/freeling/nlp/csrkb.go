...
func NewUKB(wsdFile string) *UKB {
	...
	for cfg.GetContentLine(&line) {
		...
		case UKB_RELATION_FILE:
			{
				fname := items[0]
				if strings.HasPrefix(fname, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/") {
					wsdFile = strings.Replace(wsdFile, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/", "", -1)
					path = wsdFile[0:strings.Index(wsdFile, "/")]
					relFile = path + "/" + strings.Replace(fname, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/", "", -1)
				} else {
					relFile = path + "/" + strings.Replace(fname, "github.com/sniperkit/xanalyze/plugin/nlp/freeling/", "", -1)
				}
				break
